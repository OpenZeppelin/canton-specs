<!--
  GROUNDING SOURCE OF TRUTH — stored verbatim below the banner.
  This is the approved Canton dev-fund grant proposal (PR #298). It defines the
  engagement's scope, milestones, deliverables, adoption criteria, and
  licensing. Treat it as the top-level "why / what / when" source of truth for
  all RI and library work in this workspace.
-->

# Canton Ecosystem Grant Proposal — Source of Truth

> **Status banner (workspace grounding — read before the proposal text):**
>
> - **What this is.** The OpenZeppelin "Canton Ecosystem Stack" grant proposal,
>   author `gabitu7`, **Approved 2026-05-13**, canton-foundation/canton-dev-fund
>   [PR #298](https://github.com/canton-foundation/canton-dev-fund/pull/298).
>   Stored verbatim below as the grounding source of truth for scope,
>   milestones, deliverables, adoption criteria, and licensing.
> - **CIP-56 → CIP-112 retarget (informally approved).** The proposal text names
>   **CIP-56** as the M1 token standard. Per this workspace's M1 rescope
>   (`PLAN.md` Decision Log **S1**, 2026-06-16), **CIP-56 is retargeted to
>   CIP-0112 / Token Standard V2** for *any functionality where CIP-56 was
>   expected*. This retarget is **informally approved**; the formal
>   grant-milestone amendment is still an open, non-blocking action item
>   (`PLAN.md` → Phase 1 Action Items → "Grant-milestone amendment capture").
>   Where the proposal says CIP-56 for M1 build target, **read CIP-0112 / Token
>   Standard V2**. CIP-56 remains background / migration evidence only.
> - **Stabilization expectation.** Token Standard V2 (CIP-0112) is expected to be
>   **stabilized and exposed properly soon**. Until then, design **against the
>   interfaces**, not DAR/package-ID pins, and **maximally match the Splice V2
>   standard interfaces** on the tracked branches (next bullet). This is the
>   standing instruction in
>   [`RI_RESEARCH_BRIEFING.md`](./RI_RESEARCH_BRIEFING.md).
> - **Splice V2 standard branches (tracked herein).**
>   - **Current source-of-record pin:** `hyperledger-labs/splice` branch
>     `token-standard-v2-upcoming` @ `1e34121b2b369c5dde357c098e2aaeb65250e736`.
>     Recorded in
>     [`docs/architecture/cip0112-import-source-of-record-decision.md`](../architecture/cip0112-import-source-of-record-decision.md),
>     [`docs/architecture/cip0112-splice-token-standard-v2-import-gate.md`](../architecture/cip0112-splice-token-standard-v2-import-gate.md),
>     `README.md`, and the settlement-spine source headers.
>   - **The literal "preview" branch (now historical evidence):**
>     `canton-network/splice` branch `token-standard-v2-daml-preview` @
>     `b91de5d4b910ded598151981654dce2acc6f84ba` (local working copy at
>     `/Users/x/excanton/CN/splice`). The actual V2 DAR set
>     (`splice-api-token-allocation-instruction-v2`,
>     `…-allocation-request-v2`, `…-allocation-v2`, `…-transfer-events-v2`, all
>     `1.0.0`) and their SHA-256 checksums are catalogued in
>     [`canton-token-template/docs/CIP-0112-EXTENSION-PLAN.md`](../../canton-token-template/docs/CIP-0112-EXTENSION-PLAN.md).
> - **Import is still gated.** Storing this proposal does **not** authorize
>   vendoring/importing Splice DARs or any public-API stability / conformance /
>   audit / production / release claim. The import gate stands (`PLAN.md` Open
>   Questions; import-gate ADR above).
> - **Authority precedence.** For operating rules and decision authority,
>   `AGENTS.md` and `PLAN.md` win. This proposal is the *scope/intent* source of
>   truth; where it conflicts with the CIP-56→CIP-112 retarget, the retarget
>   wins for the M1 build target.
> - **Distilled per-RI scope locks:** [`M2_DEX_SCOPE.md`](../../M2_DEX_SCOPE.md),
>   [`M3_LENDING_SCOPE.md`](../../M3_LENDING_SCOPE.md) (one
>   `M<grant-milestone>_<RI>_SCOPE.md` per RI).
...
| **M4** (Q4, End Y1) | RI 3: Cross-Chain, RI 4: Auction | Messaging Gateway, Additional Standards | Audits, Reviews, Pen Tests | Included | 7,804,054 CC | 7.5% (Delivery) / 20% (Adoption) | Committee acceptance AND adoption criteria |
...
| **M8** (Q8, End Y2) | RI 8 (TBD) | Additional Standards | Audits, Reviews, Pen Tests | Included | 9,932,433 CC | 5% (Delivery) / 30% (Adoption) | Committee acceptance AND adoption criteria |
