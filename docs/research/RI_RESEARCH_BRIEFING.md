...
| **`canton-settlement-explorer`** | Browser tool that **exports Mermaid flow‑of‑funds diagrams**, institutional summaries, and complexity/scope tiers from scenario JSON. Use to generate the report diagrams. | Working tool (Vite/React). 7 presets incl. Privacy DEX, Batch DvP, Cross‑chain Bridge. |
| **`daml-lint` / `daml-props` / `daml-verify`** | Validation stack to cite in each report's testing/auditability section: lint (6 security detectors) → property tests (conservation, supply, balance invariants) → Z3 formal proofs (conservation, division‑safety, temporal). | Working tooling. |
