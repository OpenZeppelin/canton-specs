...
One scope shift applies throughout: the proposal targeted CIP-56, which the approved Token Standard V2 upgrade (CIP-0112) superseded. Per boundary B4, the token foundation ships as the **CIP-0112 settlement primitive**, and CIP-86 / CIP-103 / CIP-104 ship as **interoperability evidence against that surface** — not standalone middleware, wallet-provider, or Scan/SV reward deliverables. CIP-56 remains migration evidence only (`canton-token-template`).
