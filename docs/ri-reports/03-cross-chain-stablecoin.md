...
> exposes only `TransferPreapproval_Send` / `TransferPreapproval_MintInto` — it
  ...
  where
    ...
    choice Execute_Inbound_Settlement : ContractId SettlementReceipt
      ...
      do
        ...
        -- RI-level extension (the evidence template only has _Send/_MintInto);
...
| `TransferPreapproval` | `canton-token-template` (`SimpleToken.Preapproval`) | Recipient-signed preapproval pattern; the spine-aware delegated-accept choice the RI uses (§4.2) is a `[FUTURE]` extension — the evidence template only ships `_Send`/`_MintInto`. | `[EVIDENCE]` (+ `[FUTURE]` extension) |
