...
the `Pool`, its pool-scoped pause state, and the trusted-issuer registry
...
`registry.admin == factoryAdmin` ([section 4.2](#42-component-d1-compliance-attestation)).
...
[`PauseState`](../../pausable/daml/OpenZeppelin/Pausable.daml#L47) is
`signatory pauser` with a single global flag, so the per-pool pause state below is
an RI-level variant of it (signed by the venue operator and keyed by the pool
tuple), and ShapeB's
[`TrustedIssuerRegistry`](../../experiments/identity-hook-shape-b/daml/OpenZeppelin/Experimental/Identity/ShapeB.daml#L84)
