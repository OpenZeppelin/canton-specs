...
- **Permissioned.** Every participant acts under a verified identity:
  ...
  the `TrustedIssuerRegistry`, and value movements require a compliance
  attestation, so no anonymous party can open, service, or liquidate a
  position.
...
carries the authority they need. The interest portion of a payment is recorded as a
...
typed trusted-issuer claims in the [identity packages](../../experiments/identity/);
...
| Pricing | A keyed `PriceOracle` naming both the collateral and quote instruments, with max-staleness and per-update deviation guards. Its update mechanism is implementation-defined, bound by the interface requirements ([section 4](#42-component-price-oracle-interface)). |
...
| Compliance & Control | D1: a settlement does not execute unless an attester has signalled compliance, re-checked on every value-moving operation. D2: a privileged party can block settlement and sweep allocation funds to a preset custodian account. D3: single-synchronizer identity. |
...
flowchart TB
    ...
    Borrower -->|"originate"| Factory
    Factory -->|"creates"| Vault
    Borrower -->|"deposit, borrow,<br/>repay, close"| Vault
    Liquidator -->|"flag, liquidate"| Vault
    Attester -.->|"single-use D1 attestation,<br/>consumed by every<br/>value-moving choice"| Vault
...
flowchart TB
    ...
    subgraph Identity["Identity (D3)"]
        Kyc["KycClaim /<br/>TrustedIssuerRegistry"]
    end

    subgraph Rail["CIP-0112 registry"]
        Rules["TokenRules (mint / burn)"]
        Settle["Allocation + SettleBatch<br/>(deposit delivery, D1-attested)"]
    end

    Vault -->|"pause gate,<br/>role checks"| Gov
    Vault -->|"live KYC fetch"| Kyc
    Vault -->|"mint, burns"| Rules
    Settle -->|"deliver committed<br/>collateral deposit"| Custody
    Settle -.->|"D2 sweep"| Custodian
```
...
- **Vault Admin / Stablecoin Issuer (`VAULT_ADMIN`)** - underwrites the **stablecoin (debt) token**: operates the `VaultFactory`, configures `VaultParams` and the trusted registries, and issues the time-bounded, case-scoped grants that authorize D2 seizure sweeps. The admin can mint the stablecoin, never the collateral, and only inside the vault choices ([section 3](#3-target-design)). The protocol gives the vault admin no alternative to issue unbacked debt token.
- **Borrower (`BORROWER`)** - the entity locking collateral and drawing debt. Only the borrower can lock their own holdings into an allocation. They must hold a valid `KycClaim`, verified at origination and fetched live by each value-moving vault choice, and every value-moving operation additionally consumes a single-use D1 attestation. Visibility is limited to their own vaults and the public configuration contracts.
...
- **Oracle Operator(s) (`ORACLE_PROVIDER`)** - the implementation-defined party set that updates the `PriceOracle`, bound by the interface requirements ([section 4](#42-component-price-oracle-interface)): no single party, not even the vault admin, can move or stall the published price.
...
the vault admin and the insurance fund. For each, the design envisions the
...
The vault custody account is envisioned as a registry `Account`
...
When such a role must submit routinely (the insurance fund collecting fees), we envision it keeping its confirmation threshold at 1 with its powers bounded on-ledger.

Whatever update mechanism the **oracle operators** run ([section 4](#42-component-price-oracle-interface)), an all-of-M quorum is deliberately avoided: a single offline member, or one whose validator has unvetted the protocol DAR, would stall every price update until the staleness guard freezes the protocol.
...
- **Underwater vault (`collateralRatio <= 1 + liquidationBonus`).** No repayment can restore health, so the cap becomes what the remaining collateral can pay for: the pass seizes all of it, and the uncovered remainder is quantified as `badDebt`, whose first absorber is the insurance fund.
...
**A. Collateral deposit.** The borrower commits the collateral they are locking; settlement delivers it into the vault's custody account, and the vault's own record of how much collateral backs the position grows by the same amount, all in one transaction. The first deposit goes through the `VaultFactory` instead: origination settles it identically, but creates the `Vault` rather than updating one ([section 4.3](#43-component-vaultfactory-and-origination)).
...
flowchart TD
    ...
    Factory -.->|"first deposit: originate,<br/>create the Vault"| Vault
```
...
1. **Origination and deposits.** The borrower locks collateral into a committed [`TokenAllocation`](https://github.com/OpenZeppelin/canton-contracts/blob/7696749737885e25cd88422847105f890f03b00d/experiments/token/tokenCIP112-v1/daml/OpenZeppelin/TokenCIP112V1/Allocation.daml#L67) and exercises `VaultFactory_Originate`, which checks the KYC claim, batch-settles the collateral into the custody account, and instantiates the `Vault` with the settled `collateralAmount`. Top-ups use `Vault_DepositCollateral`; `Vault_WithdrawCollateral` releases collateral back as long as the solvency check passes.
2. **Borrow.** `Vault_MintStablecoin` consumes the D1 attestation, fetches the live `KycClaim`, and requires that existing plus requested debt keeps `collateralRatio` at or above `minCollateralRatio` at a fresh `PriceOracle` reading, before minting to the borrower and incrementing `debtAmount`.
...
single atomic exercises under authority the vault choice already carries;
...
|---|---|---|---|
| Deposit / origination | 1. lock the collateral for settlement | borrower | sync: borrower alone |
| Deposit / origination | 2. settle the collateral into the vault's custody account, creating or updating the vault | borrower | async: has to wait for D1 compliance from attestor |
| Borrow | mint the stablecoin against the vault's collateral | borrower | async: has to wait for price update if stale, as well as D1 compliance from attestor |
...
| Liquidation | 2. liquidate: burn the payment, seize collateral | liquidator keeper | keeper alone, once the grace period lapses uncured; has to wait for price update if stale, as well as D1 compliance from attestor |
...
|---|---|---|---|
| Collateral deposit | the borrower end to end: allocate, then settle, back-to-back | minutes; extend toward the 24h prepared-transaction ceiling only when the borrower signs through an external custodian | both steps carry only the borrower's authority, so a live wallet completes them in one session; not price-sensitive, and compliance is re-checked at settle time |
| Oracle publish | oracle operators, automated | minutes | the staleness guard rejects slow publishes and the circuit breaker trips on gaps |

Consequence for D1: attestation is automated, issued just-in-time with a
short validity window, and `settlementDeadline` must exceed the attester's
SLA. The margin-call grace window ([section 3](#margin-call-a-grace-period-before-liquidation))
...
**Direct-transfer assumption.** The release legs assume the collateral registry lets a transfer complete inside the vault transaction. Both account parties are always present in a release, so even an instructed transfer completes synchronously when its acceptance is receiver-driven: the choice creates the instruction and accepts it in the same transaction. Note this assumption can break for a registry that interposes its own asynchronous step, such as a registrar acceptance or a pending state resolved by registry automation. Such an asset still integrates, but its releases split in two: the vault transaction issues the instruction, and the collateral arrives when the registry's step lands.

**Collateral amounts vs. actual holdings.** The `Vault`'s `collateralAmount` is a `Decimal` accounting figure; the real value lives in TSv2 holdings owned by that vault's own custody account. Every flow moves holdings into or out of that account in the same transaction that updates the accounting figure, so **`collateralAmount == Σ(custody-account holdings)` per vault** cannot drift within a transaction. Deposit, withdrawal, and liquidation each bind their collateral leg to the custody account's identity, so the accounting can never move without the matching holdings moving. The caveat is *fragmentation*: repeated top-ups accumulate many small holdings in a custody account, so a periodic **consolidation** step (merging the account's holdings for the instrument into one, leaving `collateralAmount` unchanged) keeps settlement cheap.

### Fees Are Burned, Then Collected

The debt paid on repay, close, or liquidation is `principal + accrued interest`; a liquidation additionally charges the borrower the `liquidationBonus`, taken in collateral and paid to the liquidator as the seizure premium. Both paths burn the full payment via the registry's `TokenRules_Burn` and record the interest portion in the vault's `feeReceivable`; the insurance fund realises that revenue later by exercising `Vault_CollectFees`, which mints up to the recorded receivable. Those fees are protocol revenue, the on-ledger analogue of interest paid to the lender, and the accumulated fund is the first absorber of any liquidation shortfall.

Interest creates a structural liquidity gap: the protocol mints only principal, yet borrowers owe principal plus interest, so aggregate debt always exceeds circulating supply by the accrued interest. The stablecoin that pays interest must come from other borrowers' minted principal or from the insurance fund's re-minted fees re-entering circulation. This is the standard CDP dynamic (MakerDAO's stability fees behave the same way); it is benign while supply circulates, but it means not every position can be repaid simultaneously ([section 7](#7-open-design-questions)).
...
the
[`TrustedAttesterRegistry`](https://github.com/OpenZeppelin/canton-contracts/blob/7696749737885e25cd88422847105f890f03b00d/experiments/token/tokenCIP112-v1/daml/OpenZeppelin/TokenCIP112V1/D1.daml#L22).
...
land as an appended `debtCeiling : Optional Decimal` on `VaultParams` and a
...
- **Par-liquidity module.** A PSM-style valve minting and redeeming stablecoin at par against an approved cash-like asset - the supply-side lever for the same gap, at the cost of importing the reserve asset's risk.
...
`collateralRatio`, and `releaseFromCustody` are informed by the `[REFERENCE]`
...
template Vault
  with
    ...
    feeReceivable : Decimal
      -- ^ Interest already burned; the insurance fund mints against this via
      -- `Vault_CollectFees`.
    params : VaultParams
  ...
  where
    ...
    choice Vault_Liquidate : ContractId Vault
      with
        ...
        paymentHoldingCid : ContractId TokenHolding     -- liquidator's stablecoin
        debtRepaid : Decimal
      ...
      do
        ...
        -- KEY: bind seizure to what the liquidator actually pays. `debtRepaid`
        -- is burned from the liquidator's own holding in this same exercise,
        -- capped at exactly the repayment that restores the vault to
        -- `minCollateralRatio`; once no repayment can restore health, the cap
        -- is full absorption. The choice itself computes `collateralToSeize`
        -- and releases exactly that amount, so there is nothing to mis-size.
        let collateralValue = collateralAmount * oracle.price
            repayCap =
              if collateralValue > (1.0 + params.liquidationBonus) * accruedDebt
                then (params.minCollateralRatio * accruedDebt - collateralValue)
                       / (params.minCollateralRatio - 1.0 - params.liquidationBonus)
                else collateralValue / (1.0 + params.liquidationBonus)
        ...
        -- D1 inline, then burn the payment under the choice's own authority:
        -- the admin (a vault signatory) and the liquidator (the controller)
        -- are exactly the burn's required parties. No allocation or batch.
        verifyAndConsumeAttestation attestationCid liquidator
        liqHolding <- fetch paymentHoldingCid
        assertMsg "payment holding must match debtRepaid"
          (liqHolding.holding.amount == debtRepaid)
        exercise stablecoinRulesCid TokenRules_Burn with
          holdingCid = paymentHoldingCid
          account = liqHolding.holding.account
        ...
        create this with
          ...
          -- The non-principal portion is protocol revenue, realised later by
          -- the insurance fund through `Vault_CollectFees`.
...
Candidate mechanisms include an N-of-M committee collecting approvals through the [Multiple Party Agreement](https://docs.canton.network/appdev/modules/m3-design-patterns#multiple-party-agreement) pattern, medianized per-member price submissions, pull-style signed price attestations verified inside the consuming choice, and a delayed price in the style of MakerDAO's oracle security module.
...
template PriceOracle
  ...
  where
    ...
    -- Update choices are implementation-defined (section 7). Whatever the
    -- mechanism, it archives and recreates the oracle under the same key and
    -- must satisfy the requirements above; the deviation bound is read from
    -- `this.maxDeviation` (trusted signed state), never caller-supplied.
```

### 4.3 Component: VaultFactory and Origination

The `VaultFactory` is the vault admin's standing origination offer: an
admin-signed, keyed contract that publishes the offered terms (`VaultParams`
and the instrument pair) and lets a prospective borrower originate
unilaterally under them. The borrower controls the origination choice and
the factory contributes the admin signature, so the jointly-signed `Vault`
needs no per-borrower action from the admin. Origination is pause- and
KYC-gated and settles the committed initial collateral deposit in the same
transaction, so a vault is never created empty. The ledger's contract-key
uniqueness rejects a duplicate `(vaultAdmin, borrower, vaultId)`.
`requireLiveKycClaim` and `requireJointAccount` are illustrative helpers in
the style of section 4.1.

```daml
module OpenZeppelin.Experimental.Lending.Factory where

import OpenZeppelin.TokenCIP112V1
import Splice.Api.Token.HoldingV2 (Account, InstrumentId)
import Splice.Api.Token.AllocationV2 (SettlementInfo, TransferLeg)
import OpenZeppelin.PausableV1 (PauseState, whenNotPaused)
import OpenZeppelin.Experimental.Lending.Vault

-- | The vault admin's standing origination offer. `prospects` is the
-- observer set: the parties who can read the terms and take the offer.
template VaultFactory
  ...
  where
    ...
    nonconsuming choice VaultFactory_Originate : ContractId Vault
      with
        borrower : Party
        vaultId : Text
        custodyAccount : Account          -- joint: vaultAdmin + borrower
        initialCollateral : Decimal
        kycClaimCid : ContractId KycClaim
        -- The committed collateral deposit and its settlement context.
      ...
      do
        ...
        (_, pause) <- fetchByKey @PauseState vaultAdmin
        whenNotPaused pause
        assertMsg "initial collateral must be positive" (initialCollateral > 0.0)
        -- D3: a live claim for this borrower from a trusted issuer.
        requireLiveKycClaim kycClaimCid borrower vaultAdmin
        -- One custody account per vault, held under joint authority.
        requireJointAccount custodyAccount vaultAdmin borrower

        -- Deliver the committed deposit into custody. The D1 attestation is
        -- verified and consumed inside the batch.
        ...
        -- Fails on a duplicate (vaultAdmin, borrower, vaultId) key, so a
        -- vault identity can never be originated twice.
        create Vault with
...
- **Debt conservation (no unbacked issuance)**:
  - Stablecoin is minted only inside `Vault_MintStablecoin`, atomically with a solvency-checked `debtAmount` increment, or inside `Vault_CollectFees`, capped by the `feeReceivable` recorded when the matching interest was burned; it is burned only against a `debtAmount` decrement. There is no standalone admin mint, so circulating supply never exceeds recorded vault debt. Holds against every party except the full N-of-M admin quorum, since Daml gates creation by signatories, not choices ([section 3](#collateral-is-custodied-stablecoin-is-minted-against-debt)).
- **Seizure is payment-bound**:
  - Liquidation seizes collateral exactly proportional to the stablecoin the liquidator pays, with `debtRepaid` burned from the liquidator's own holdings in the same exercise and the release computed and executed by the choice itself. A liquidator can never take more than their payment (plus bonus) buys.
...
| Unauthorized admin action | An attacker with the admin key tries to mint unbacked stablecoin, drain custodied collateral, or invoke a D2 seizure. | Minting is reachable only through the solvency-coupled vault choice; custody holdings carry the borrower's signature too, so the admin alone cannot move them outside a vault choice; seizure requires an admin-issued, time-bounded sweep capability and sweeps only to the preset custodian account. These authorities are unforgeable contract instances under Daml's authorization model. |
...
Bounded custody caps the loss, not the inconvenience: a borrower whose
counterparties stall still waits out `settlementDeadline`, and a stale oracle
freezes the book for everyone. Chronic staleness or slow settlement is an
operational and competitive problem for the protocol before it is a safety
...
- The dominant recurring cost is not a vault flow at all: a push-model
  oracle publish costs one to several transactions per update, depending on
  the chosen mechanism ([section 7](#7-open-design-questions)), on a
  continuous cadence paid by the operators. The unfunded cadence is already
  an open question
  ([section 7](#7-open-design-questions)); app rewards do not close it,
  because the admin's credit on its own publishes rebates its own burn
  ([section 6.2](#62-app-rewards)).
...
to the Global Synchronizer Foundation) is the vault admin and stablecoin
issuer: it signs every `Vault`, the oracle, and the pause state, and it
administers the settlement factory. The report already notes that a
value-critical N-of-M party cannot submit Ledger API commands directly
([section 2](#decentralization-and-trust-topology)); reward collection is
unaffected, landing on the wallet automation of the provider's validator.
...
| Origination or deposit settle | borrower | vault admin (signs the `Vault` and the factory-side legs) |
...
| `Vault_Liquidate` | liquidator | vault admin, confirming the vault, the custody release, and the burn |
| Oracle publish | oracle operators | the update's signatories; a rebate on the protocol's own spend, not funding |
...
fraction of each transaction's own burn, so the interest and liquidation
bonus ([section 3](#fees-are-burned-then-collected)), not rewards, carry the
...
- **Oracle update mechanism.** The interface requirements ([section 4](#42-component-price-oracle-interface)) admit several mechanisms: an N-of-M committee via the Multiple Party Agreement pattern, medianized per-member submissions, pull-style signed price attestations verified in-choice, or an OSM-style delayed price. Open: which mechanism, and its quorum sizing balancing liveness against collusion.
...
- **Guaranteed liquidatability ahead of bad debt.** A position only stays solvent for the protocol if it can be liquidated before its collateral value falls under its debt, and several design choices work against that: the buffer between `minCollateralRatio` and `liquidationRatio` plus the margin-call grace period define how much adverse price movement a position must survive before a keeper may act, the health-restore cap returns a vault only to `minCollateralRatio`, so a steadily falling price forces repeated flag, grace, and liquidation rounds, each granting a fresh cure window, the staleness guard and circuit breaker block liquidation exactly when prices move fastest, and the `liquidationBonus` net of traffic costs ([section 6](#6-network-economics-traffic-costs-and-app-rewards)) puts a floor under the vault size a keeper will touch. Open: how to size the ratio buffer and grace window against the collateral's volatility, whether per-vault or aggregate debt ceilings are needed, whether the breaker should keep liquidation open on a stale-but-bounded price (or freeze borrows before it freezes liquidations), and what stress evidence shows expected bad debt staying within the insurance fund's capacity.
...
- **Oracle delivery model: push vs request-driven.** The design assumes a push model (Chainlink price-feed style): the operators publish on their own cadence and consumers `fetchByKey` the latest in-band price, gated by the staleness assert. The alternative is request-driven (Chainlink VRF / Pyth-pull style): a vault operation first creates a price request, the operators respond with a fresh attestation, and the action executes against that response in a follow-up transaction. Open: which model the operators run. Push makes every vault choice single-transaction but forces the operators to publish continuously (and fund it, see update economics below); request-driven prices only on demand and always fresh, but splits borrow and liquidation into two round-trips, adds a liveness dependency on operator response time inside the margin-call window, and needs replay scoping so one response cannot serve two actions.
- **Oracle hardening beyond the update quorum and breaker.** Whether to also require a TWAP (and its window) and/or multiple independent feeds, and where the bounds live (`VaultParams` vs a separate oracle-policy contract).
- **Oracle update economics.** Publishing has no inherent on-ledger incentive; whether high-frequency updates need a carve-out from the interest revenue to fund them is open.
