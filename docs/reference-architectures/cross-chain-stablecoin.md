...
Source-grounding tags used throughout: `[IMPLEMENTED]` real code in this workspace, `[LIBRARY]` a reusable package in [`canton-contracts`](https://github.com/OpenZeppelin/canton-contracts), `[EVIDENCE]` real code in an evidence repo ([`canton-token-template`](https://github.com/OpenZeppelin/canton-token-template), [`canton-stablecoin`](https://github.com/OpenZeppelin/canton-stablecoin)) but not the M1 surface, `[UPSTREAM]` Splice / CIP / external-ecosystem reference, `[FUTURE]` proposed RI-level design, not built in M1 scope.
...
Provenance of this path: steps 2 and 4 run on the `[IMPLEMENTED]` spine, and everything at the cross-chain boundary — the gateway, the attestation carrier, and the nonce registry — is `[FUTURE]`. The tags below mark each item at first mention.
...
2. **Request and gate.** The relayer drives [`SettlementFactory_CreateAllocationRequest`](../../experiments/settlement/cip-0112/daml/OpenZeppelin/Experimental/Settlement/Cip112.daml#L151) `[IMPLEMENTED]`, an executor-signed request naming the mint leg with exactly the attested amount. Identity is checked on-ledger and fails closed: the recipient must hold a valid, unexpired `KycClaim` from an issuer in the `TrustedIssuerRegistry` (D3, both `[IMPLEMENTED]`, though the choice that *performs* this check is the `[FUTURE]` gateway's), and the settlement itself will require a compliance attestation from a trusted attester (D1, `[IMPLEMENTED]`). No valid claim or attestation, no credit, full rollback.
...
### D1: Compliance through Party-Applied Attestation `[IMPLEMENTED]`
...
### D2: Seizure through Preset Custodian Lock-and-Sweep `[IMPLEMENTED]`
...
- The atomic settlement primitive `[IMPLEMENTED]` serves any system that needs multi-leg DvP; the DEX and Lending RIs ride the same entrypoint, with no parallel settlement path.
...
- The `KycClaim` / `TrustedIssuerRegistry` identity hook `[IMPLEMENTED]` is the substitution point for richer identity regimes, including the deferred cross-domain D3 `[FUTURE]`.
...
- **Conservation of funds `[IMPLEMENTED]`**:
...
- **Privacy partitioning `[IMPLEMENTED]`**:
...
- **Non-custodial recipient binding**:
  - No allocation binds a recipient without their signature `[IMPLEMENTED]` supplied live, or through their standing `TransferPreapproval` `[EVIDENCE]` via a delegated choice that is `[FUTURE]`.
  - Stalled committed value is recoverable after the settlement deadline `[IMPLEMENTED]`; the RI's mandate that every committed inbound allocation carry a finite `settlementDeadline` is `[FUTURE]` policy, not a check the spine enforces.
...
| Toxic or spam inflow | A sender forces a settlement onto an unwilling recipient. | `[IMPLEMENTED]` Without the recipient's accept (live, or via their standing `TransferPreapproval` `[EVIDENCE]`), the allocation never commits; unsettled allocations expire and return to sender. |
| Compromised admin key | A compromised Stablecoin Admin or Custodian key attempts arbitrary expropriation. | `[IMPLEMENTED]` D2 sweeps are hardcoded to the preset `custodianDestination` (no arbitrary burn, no return-to-sender). `[FUTURE]` Supply-changing authority is slated for N-of-M multisig ([section 2](#decentralization-and-trust-topology)); today a single key holds it. |
...
| UTXO fragmentation | Many small transfers accumulate holding dust. | `[IMPLEMENTED]` Settlement returns a sender's surplus as a single new *change* holding per instrument rather than many fragments. |
