...
For the roles that hold value-moving or supply-changing authority - the **Stablecoin Admin** (it authors `wTOK` mint legs) and the **Custodian** (it can sweep locked value) - the design envisions the EVM equivalent of an **N-of-M multisig**: no single key may exercise the role's authority. That threshold gates the role's *authoring* transactions - creating the `SettlementFactory`, minting `wTOK`, signing the admin's SenderSide `Allocation` - but not the settle submission: [`SettlementFactory_SettleBatchWithAttestation`](../../experiments/settlement/cip-0112/daml/OpenZeppelin/Experimental/Settlement/Cip112.daml#L218) is controlled by the executors, and the admin's authority reaches [`Allocation_SettleInBatch`](../../experiments/settlement/cip-0112/daml/OpenZeppelin/Experimental/Settlement/Cip112.daml#L457) from the admin-signed factory contract rather than from a live key. Multisig therefore bounds what value can be minted and committed, not who submits the settle that moves it; narrowing that standing delegation (a shorter-lived or per-batch factory, or an admin-held settle capability) is part of the open multisig question ([section 6](#6-open-design-questions)). Canton offers two ways to implement this (which one is currently left as an open question, [section 6](#6-open-design-questions)):
...
flowchart TD
    ...
    Issuer -.->|"admin-signed Allocation, pre-committed: mint-leg SenderSide"| Settle
...
- **Multisig implementation for value-critical roles.** The Stablecoin Admin and Custodian each require N-of-M authority ([section 2](#decentralization-and-trust-topology)). Open: whether each role uses the on-ledger approval workflow, an external party with threshold signing keys, or a combination, the N and M per role, and whether the factory's standing admin delegation at settle should be narrowed so a threshold is also required per settled batch and not only when the admin's allocation is authored.
