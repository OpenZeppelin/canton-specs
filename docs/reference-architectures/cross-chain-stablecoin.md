...
1. Privacy through per-party projection `[EXPERIMENT]`: a *counterparty* sees only the legs on which they are the sender or receiver, so one recipient's payment is never visible to another. The issuing admin of the settled instrument is the deliberate exception: it signs that instrument's holdings and allocations, so it sits *inside* the trust boundary rather than outside it (see [Privacy and Visibility Model](#privacy-and-visibility-model)).
2. D1: Compliance through Party-Applied Attestation `[EXPERIMENT]` - compliance is checked per settlement, with no caching. Failure to adhere to compliance results in no credit, once the registry pins an attester registry ([D1](#d1-compliance-through-party-applied-attestation-experiment)).
3. D2: Seizure through Preset Custodian Lock-and-Sweep `[EXPERIMENT]` - a privileged party can sweep the funds in a locked allocation to a preset custodian account, and a sweep past the settlement deadline additionally needs an order signed by a registry-listed authority.
4. D3: Identity through Trusted-Issuer KYC `[EXPERIMENT]` (this workspace, ShapeB; not part of the settlement implementation above) - a recipient must hold a `KycClaim` from an issuer in the `TrustedIssuerRegistry` to receive a compliance-gated inflow. The choice that enforces the check on the inbound path is `[FUTURE]`.
5. D4: Authority through Per-Role Privilege Transfer `[FUTURE]` - the design binds each privileged action to a named role rather than to a single admin. Privileges can be transferred, granted or revoked. This capability comes from [`openzeppelin-access-control-v1`](https://github.com/OpenZeppelin/canton-contracts/tree/cec416d6e3c2118551c761d5598c403ab27ee342/experiments/access/access-control-v1) `[EXPERIMENT]`.
