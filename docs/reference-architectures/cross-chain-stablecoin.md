...
| Tag | Meaning |
|---|---|
| `[EXPERIMENT]` | experimental code, in this workspace or in `OpenZeppelin/canton-contracts` `experiments/`; the owning repository is named at first mention |
...
Four capability gates sit on top of settlement. D1 and D2 come from that same implementation, D3 from this workspace, and D4 from the access-control library.
...
Canton has no globally visible mint. The inbound gateway message drives an isolated, recipient-targeted allocation instead of broadcasting a supply change, and the atomic DvP archives the inbound request, credits the recipient's holding, and emits holdings-change events through [`TransferEventsV2.EventLog`](https://github.com/OpenZeppelin/canton-contracts/blob/7696749737885e25cd88422847105f890f03b00d/experiments/token/tokenCIP112-v1/daml/OpenZeppelin/TokenCIP112V1/Base.daml#L80) to the recipient, the relayer, and the issuing admin.

Two consequences shape the design. A recipient's own signature, live or delegated, is required to bind them, so the propose-and-accept handshake is a necessity rather than a style choice. And the `PauseState`, the trusted-issuer registry, and the consumed-nonce registry all need a lookup handle that survives archive-and-recreate, which means contract keys `[GAP]`. Keys returned in [Canton 3.5](https://github.com/digital-asset/canton/releases/tag/v3.5.1) `[UPSTREAM]`, and three costs come with adopting them:

- **Toolchain and LF.** Keys need Daml-LF 2.3, which needs Protocol Version 35, and their use requires `HASHING_SCHEME_VERSION_V3`, which the externally signed submissions of [section 2](#decentralization-and-trust-topology) also require. Adoption is therefore a synchronizer protocol-version migration that every participant on the rail must complete, not a package-level change. The workspace baseline is 3.4.11 with no key support, so each `[EXPERIMENT]` choice today takes a caller-supplied registry contract id and asserts that the registry shares the factory's admin. The workspace also targets LF 2.1 because the vendored Splice V2 API DARs do, so moving the OpenZeppelin package to 2.3 puts it on a different LF from every package it depends on. Depending downwards is generally permitted, but the RI must confirm it against whichever SDK it selects rather than assume it.
- **Templates.** No template in this workspace declares a `key`, so by-key resolution is a template change as well as a toolchain migration.
- **Migration.** Adding a key to an existing template is not SCU-legal: the [Canton 3.5.1 release notes](https://github.com/digital-asset/canton/releases/tag/v3.5.1) `[UPSTREAM]` forbid adding or removing a key definition in a later version of a template, and package vetting enforces that at upload on a real participant rather than at compile time. Of the three templates only `ConsumedNonceRegistry` can be born keyed. `PauseState` is released code, pinned as `dars/vendor/openzeppelin-pausable-v1-0.1.0.dar`, and `TrustedIssuerRegistry` is a live experimental template, so keying either one means a new template in a new package lineage plus migration of every active contract.

A key's maintainers must be signatories of the keyed contract, so each key is fixed by that contract's own authority rather than by the rail `admin`: [`PauseState`](https://github.com/OpenZeppelin/canton-contracts/blob/cec416d6e3c2118551c761d5598c403ab27ee342/experiments/security/pausable-v1/daml/OpenZeppelin/PausableV1.daml#L47) `[EXPERIMENT]` is `signatory pauser`, so its key is `pauser`, and ShapeB's [`TrustedIssuerRegistry`](../../experiments/identity/hook-shape-b/daml/OpenZeppelin/Experimental/Identity/ShapeB.daml#L74) `[EXPERIMENT]` is `signatory registryAdmin`, so its key is `registryAdmin`. The diagrams and snippets below use those maintainers. The trusted-attester registry is the deliberate exception: its contract id is pinned on the settlement registry as `requiredAttesterRegistryCid`, so nothing is resolved and no caller names it.
...
| Holdings & Preapproval `[EVIDENCE]` | `canton-token-template` (`SimpleToken.*`): `SimpleHolding`, `SimpleTokenRules`, `LockedSimpleHolding`, `TransferPreapproval` | Asset representation and the recipient-signed standing-delegation pattern. The spine-aware delegated-accept choice is a `[FUTURE]` extension ([section 4.2](#42-component-inbound-dvp-via-delegated-accept-future)); the evidence template ships only `TransferPreapproval_Send`. |
| Messaging Gateway `[FUTURE]` | `StandardizedMessagingGateway` (bounded mock, [section 4.1](#41-component-standardized-messaging-gateway-bounded-mock-future)) | Cross-chain boundary: consumes attester-signed inbound messages and drives the spine. |
...
Canton [decentralizes](https://docs.canton.network/overview/reference/decentralization) a party along three independent axes `[UPSTREAM]`: **governance** (whose signatures can change the party's identity and hosting), **validation** (how many independent validators must confirm its transactions, set by the `PartyToParticipant` confirmation threshold), and **authorization** (what the Daml signatory and controller topology requires regardless of hosting). A confirmation threshold above 1 defends against a malicious validator at a latency and cost premium, and such a party can no longer submit Ledger API commands directly: it acts through externally signed submissions or through choices that others submit.
...
| `wTOK` holding (`TokenHolding` in the experiment) | the instrument admin, the account's parties | the lock's observers, when locked |
...
- **Conservation of funds `[EXPERIMENT]`.** Settlement cannot output more value than its input allocations: on every settle path the engine archives the locked input holdings and asserts, per instrument, that they cover the authorizer's SenderSide leg amounts, returning any surplus as a single unlocked change holding. (`nextIterationFunding` is inert forward-compatible metadata; M1 implements no iterated settlement, so no path defers conservation.)
...
| **Multisig for value-critical roles.** Open: whether the Stablecoin Admin and the Custodian each use the on-ledger approval workflow, an external party with threshold signing keys on `PartyToParticipant`, or a combination, and the N and M per role. | party onboarding for both roles; today a single key holds each | **High** - a compromised key is unmitigated until it lands | internal team |
