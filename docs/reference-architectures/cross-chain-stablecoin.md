...
| Holdings and standing `TransferPreapproval` | `[EVIDENCE]` | [`canton-token-template`](https://github.com/OpenZeppelin/canton-token-template) | `TransferPreapproval_AllocateInbound`, a delegated choice that allocates on the settlement spine under the recipient's standing signature; the template's own `TransferPreapproval_Send` only sends a transfer ([section 4.2](#42-component-inbound-dvp-via-delegated-accept-future)) |
